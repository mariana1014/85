...
export default class RideScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      Alert.alert("Favor de escanear/escribir un id de bicicleta válido.");
      ...
      if (isEligible) {
        ...
        Alert.alert(
          "Rentaste la bicicleta la siguiente hora (1). ¡Disfruta tu viaje!"
        );
        this.setState({
          bikeAssigned: true
        });
        ...
        // ToastAndroid.show(
        //   "Rentaste la bicliteta la siguiente hora (1). ¡Disfruta tu viaje!",
        //   ToastAndroid.SHORT
        // );
      }
    } else {
      ...
      if (isEligible) {
        ...
        Alert.alert("Esperamos que haya disfrutado su viaje.");
        this.setState({
          bikeAssigned: false
        });
    ...
    if (userRef.docs.length == 0) {
      ...
      userRef.docs.map(doc => {
        if (!doc.data().bike_assigned) {
          ...
          isUserEligible = false;
          Alert.alert("Termina tu viaje actual para rentar otra bicicleta.");
          this.setState({
            bikeId: ""
          });
        }
  ...
  checkUserEligibilityForEndRide = async (bikeId, userId, email) => {
    ...
    transactionRef.docs.map(doc => {
      ...
      if (lastBikeTransaction.user_id === userId) {
        ...
        isUserEligible = false;
        Alert.alert("Otro usuario rentó esta bicicleta.");
        this.setState({
          bikeId: ""
        });
      }
  ...
  assignBike = async (bikeId, userId, bikeType, userName, email) => {
    ...
    //Cambia el estatus de la bicileta.
    db.collection("bicycles")
      .doc(bikeId)
      .update({
